...
function setUserData(data): void {
  allSettings.userAvatar = data.avatarUrl;
  allSettings.username = data.tag;
  fs.writeFileSync(settingsFile, JSON.stringify(allSettings), 'utf8');
}
...
async function sendTestWebhook(testHook): Promise<void> {
  ...
  const options = {
    ...
    json: {
      username: 'das',
      avatar_url: 'https://i.ibb.co/dmjd1Xq/Unicorn-Face-1-F984.png',
      embeds,
    },
  };
  try {
  ...
  const embeds = [
    {
      ...
      author: {
        name: `asd${webhookData.purchaseType === 'decline' ? 'Purchase Declined ❌' : webhookData.purchaseType === '3ds' ? 'Waiting 3DS Comfirmation ⏱️' : 'Purchase Success ✔️'}`,
        // url: 'https://discordapp.com',
        icon_url: 'https://i.ibb.co/dmjd1Xq/Unicorn-Face-1-F984.png',
      },
      footer: {
...
async function sendSetupWebhook(setupData): Promise<void> {
  ...
  const options = {
    method: 'POST',
    uri: 'https://discordapp.com/api/webhooks/735178680095211529/lhk4NpslCE2VUx8bKhkjCs2hDEVcvHNi7tMo49csXodCQQA--qpwOUL2odk7W-Xuvjep',
    json: {
      username: 'qweqwe',
      avatar_url: 'https://i.imgur.com/pxxznJG.png',
      embeds,
