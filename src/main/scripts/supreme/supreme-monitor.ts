...
class SupremeMonitor extends MainMonitor {
  ...
  async getStyles(): Promise<void> {
    ...
    try {
      const response = this.stylesFromUrl
        ? null
        : await this.requests.getProductData(`${this.productData.productURL}.json`);
      ...
      const colors = styles.filter(style =>
        style.name.toLocaleLowerCase().includes(this.taskData.productColor.toLocaleLowerCase()),
      );
      if (!colors.length) {
