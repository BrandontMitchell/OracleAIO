...
class ShopifyAdvanced extends ShopifyMain {
  ...
  async waitQueue(): Promise<any> {
    ...
    while (status === 202 && !this.stopped) {
      try {
        ...
        location =
          pollResponse.headers.location.indexOf('https') !== -1
            ? pollResponse.headers.location
            : this.taskData.site + pollResponse.headers.location;
      } catch (error) {
  ...
  async addToCart(): Promise<void> {
    ...
    try {
      ...
      if (
        (error.message.includes('not_enough_in_stock') ||
          error.message.includes('Cannot find variant') ||
  ...
  async submitPayment(): Promise<void> {
    ...
    try {
      ...
      if (this.paymentResponse && this.paymentResponse.statusCode === 200) {
        this.sendStatus(
          `${taskStatus.submittingPayment} ${this.paymentSubmissions === 0 ? '' : this.paymentSubmissions}`,
          taskColors.yellow,
        );
  ...
  async handlePayment(): Promise<void> {
    ...
    try {
      if (
        ...
        this.paymentResponse.body.indexOf(
          'The total of your order has changed. Review the order summary before you continue.',
        ) !== -1
      ) {
        ...
        this.paymentResponse.body
          .toLowerCase()
          .indexOf('your cart has been updated and the previous shipping rate isn’t valid') !== -1
      ) {
        ...
        (this.paymentResponse.headers.location &&
          this.paymentResponse.headers.location.indexOf('stock_problems') !== -1)
      ) {
        /**
        ...
        this.paymentResponse.request.path.indexOf('processing') !== -1 ||
        (this.paymentResponse.headers.location && this.paymentResponse.headers.location.indexOf('processing') !== -1)
      ) {
        ...
        this.paymentResponse.body.indexOf('attributes[sp_suspicious]=false') !== -1 &&
        !this.orderPayload['checkout[attributes][sp_token]']
      ) {
    ...
    try {
      ...
      while (
        ...
        (payments[payments.length - 1].transaction === null &&
          !payments[payments.length - 1].checkout.order &&
          !this.stopped)
      ) {
        if (this.stopped) {
          return;
        ...
        if (payments[payments.length - 1].payment_processing_error_message) {
          this.log(
            'fatal',
            this.taskData.id,
            `Checkout error ${payments[payments.length - 1].payment_processing_error_message}`,
          );
          this.checkpoints['Submitted payment'] = false;
          this.paymentSubmissions += 1;
          this.sendWebhook({
  ...
  saveCustomerInformation(): void {
    ...
    // billing
    customerInfo.checkout.billing_address.address1 = profile.usebilling
      ? profile.billing.address
      : profile.shipping.address;
    customerInfo.checkout.billing_address.address2 = profile.usebilling
      ? profile.billing.apt
      : profile.shipping.apt || '';
    customerInfo.checkout.billing_address.city = profile.usebilling ? profile.billing.city : profile.shipping.city;
    customerInfo.checkout.billing_address.country = profile.usebilling
    ...
    customerInfo.checkout.billing_address.province = profile.usebilling
  ...
  savePaymentPayload(): void {
    ...
    paymentPayload['checkout[different_billing_address]'] = profile.usebilling
      ? profile.usebilling.toString()
      : 'false';
    ...
    if (profile.usebilling) {
      paymentPayload['checkout[billing_address][first_name]'] = profile.usebilling
        ? profile.firstname
        : profile.firstname;
      paymentPayload['checkout[billing_address][last_name]'] = profile.usebilling ? profile.lastname : profile.lastname;
      paymentPayload['checkout[billing_address][address1]'] = profile.usebilling
        ? profile.billing.address
        : profile.shipping.address;
      paymentPayload['checkout[billing_address][address2]'] = profile.usebilling
        ? profile.billing.apt
        : profile.shipping.apt || '';
      paymentPayload['checkout[billing_address][city]'] = profile.usebilling
        ? profile.billing.city
        : profile.shipping.city || '';
      paymentPayload['checkout[billing_address][country]'] = profile.usebilling
        ? profile.billing.country
        : profile.shipping.country;
      paymentPayload['checkout[billing_address][province]'] = profile.usebilling ? profile.billing.state : '';
      paymentPayload['checkout[billing_address][zip]'] = profile.usebilling
        ? profile.billing.zip
  ...
  checkCart(checkoutObject): boolean {
    ...
    if (
      (!checkoutObject.checkout ||
        !checkoutObject.checkout.line_items ||
        checkoutObject.checkout.line_items.length === 0) &&
      !this.stopped
    ) {
      return false;
    }
  ...
  async handleError(message, error, method): Promise<void> {
    ...
    if (
      message.indexOf('ESOCKETTIMEDOUT') !== -1 ||
      message.indexOf('Too many requests') !== -1 ||
      message.indexOf('Page temporarily unavailable') !== -1
    ) {
